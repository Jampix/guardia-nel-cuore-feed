...
|---|---|---|---|
| `GET /categories` | `categories.ts` | pubblica | categorie attive |
| `GET /feedback/public` | `list-public-feedback.ts` | pubblica | bacheca (GSI `byVisibilita`); `notaInterna` rimossa, `fotoUrl` prefirmato |
| `POST /feedback` | `create-feedback.ts` | autenticata | crea proposta (stato iniziale `proposta`) |
...
| `PATCH /admin/feedback/{id}` | `patch-feedback.ts` | staff | moderazione: `stato` / `rispostaPubblica` / `notaInterna`; al cambio stato **email** all'autore (SES) |
